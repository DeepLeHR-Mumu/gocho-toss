...
import { GOCHO_DESKTOP_URL, GOCHO_MOBILE_URL, SPEC_DETAIL_META } from "shared-constant";
...
export const PageHead: FunctionComponent<PageHeadProps> = ({ option }) => {
  ...
  return (
    <Head>
      <title>{`${userName}님의 생산직 스펙평가 - 고초대졸닷컴`}</title>
      <meta
        name="description"
        content={`${option.age}세 ${option.gender} ${option.desiredTask && option.desiredTask} 구직자, ${
          option.certificate && `자격증 : ${option.certificate}`
        } . ${userName}님의 생산직 스펙은 몇점일까요?`}
      />
      <meta
        name="keywords"
        content={`${option.age}, ${option.gender}, ${option.desiredTask && option.desiredTask}, ${
      ...
      <meta
        ...
        content={`${option.age}세 ${option.gender} ${
