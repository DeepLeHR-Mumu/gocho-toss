...
export const FactoryBox: FunctionComponent<FactoryBoxProps> = ({ index, register, watch, remove }) => {
  return (
    <li css={factoryContainer}>
      <h3 css={factoryTitle}>공장 정보</h3>
      <div css={inputContainer}>
        <strong css={inputTitle}>공장 이름 *</strong>
        <input css={inputBox} {...register(`factories.${index}.factory_name`, { required: true })} />
      </div>
      <div css={inputContainer}>
        <strong css={inputTitle}>공장 주소 *</strong>
        <input css={inputBox} {...register(`factories.${index}.address`, { required: true })} />
      </div>
      <div css={inputContainer}>
        <div css={welfareBox}>
          <strong css={inputTitle}>남자 임직원 *</strong>
          <input type="number" css={inputBox} {...register(`factories.${index}.male_number`, { required: true })} />
        </div>
        <div css={welfareBox}>
          <strong css={inputTitle}>여자 임직원 *</strong>
          <input type="number" css={inputBox} {...register(`factories.${index}.female_number`, { required: true })} />
        </div>
      </div>
      <div css={inputContainer}>
        <strong css={inputTitle}>생산품 *</strong>
        <input css={inputBox} {...register(`factories.${index}.product`, { required: true })} />
      </div>
      <div css={inputContainer}>
      ...
      <div css={inputContainer}>
        ...
        <label css={inputLabel} htmlFor={`기숙사유무${index}`}>
          <input type="checkbox" id={`기숙사유무${index}`} {...register(`factories.${index}.dormitory_bool`, {})} />
